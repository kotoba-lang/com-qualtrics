(ns qualtrics.main
  "Kotodama WASM entrypoint for the Qualtrics clean-room actor (L4) — Clojure port.
  ...
  facts (`qualtrics.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "qualtrics")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"qualtrics.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "qualtrics-compat" :tier tier :entities entities} 200])
...
(defn start! [] :qualtrics-compat/ready)
